<template>
  <div>
    <el-form label-width="100px" :model="formData" :rules="rules" ref="formNode" status-icon >

      <el-form-item label="公司简介" prop="introduce" :disabled="!isEditStatus">
      <el-input type="textarea" :rows="5" placeholder="请输入公司简介" v-model="formData.introduce"></el-input>
      </el-form-item>

      <el-form-item label="图片" prop="imgUrl">
        <div v-if="isEditStatus">
          <div>
            <img :src="formData.imgUrl" height="100" alt />
          </div>
          <el-button class="file-upload" type="primary" v-if="fileData == null">
            上传
            <i class="el-icon-upload el-icon--right"></i>
            <label class="file-upload-label">
              <input
                type="file"
                ref="fileNode"
                accept=".jpg, .jpeg, .png, .gif"
                @change="uploadFun"
              />
            </label>
          </el-button>
          <el-tag v-else closable type="info" @close="()=>{fileData = null;}">{{ fileData.name }}</el-tag>
          <div v-show="isShowFileError" class="el-form-item__error">请输选择图片文件</div>
        </div>
        <img v-else :src="formData.imgUrl" height="100" alt />
      </el-form-item>

      <el-form-item class="contact-add">
        <el-button v-if="!isEditStatus" type="primary" @click="isEditStatus = true;">编辑</el-button>
        <el-button v-if="isEditStatus" @click="close()">返回</el-button>
        <el-button v-if="isEditStatus" type="primary" @click="submit()">修改</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>

<script>
import { getIndexUs, putIndexUs } from "@/api/index";

export default {
  data() {
    return {
      formData: {
        introduce: "",
        imgUrl: ""
      },
      fileData: null,//记录文件对象
      isShowFileError: false,//是否显示文件的错误提示
      isEditStatus: false,//true表示是在编辑状态，false表示查看状态
      rules: {
        introduce: [{ required: true, message: "请输入公司简介", trigger: "blur" }],
        imgUrl: [{ required: true, message: "请插入图片", trigger: "blur" }]
      }
    };
  },
  created() {
    this.getIndexUs();
  },
  methods: {
    close() {
      this.isEditStatus = false;
      this.isShowFileError = false;
      this.getIndexUs();
      this.$refs.formNode.clearValidate();
    },
    submit() {
      let isValid = false; //false校验不通过，true表示通过
      if (this.fileData) {
        isValid = true;
        this.isShowFileError = false;
      } else {
        this.isShowFileError = true;
      }
      this.$refs.formNode.validate(valid => {
        if (valid && isValid) {
          //进入if表明所有的字段都符合输入要求
          var data = new FormData();
          data.append("file", this.fileData); //写入文件
          data.append("introduce", this.formData.introduce);
          putIndexUs({
            data
          }).then(res => {
            this.$message({ 
                message: res.data.msg,
                type: "success"
              });
              this.colse();
            }).catch(err => {
              this.$message({
                message: res.data.msg,
                type: "error"
              });
            });
        }
      });
    },
    uploadFun(e) {
      var file = e.target.files[0];
      this.fileData = file;
      this.isShowFileError = false;
    },
    getIndexUs() {
      getIndexUs({}).then(res => {
          var data = res.data.data;
          this.formData = data;
        }).catch(err => {
          console.log(err);
        })
    },
  },
  watch: {

  },
};
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped lang="scss">
.contact-add {
  text-align: right;
}
/deep/.none-line-height .el-form-item__content {
  line-height: inherit;
}

.file-upload {
  position: relative;
  overflow: hidden;
  .file-upload-label {
    position: absolute;
    left: 0;
    top: 0;
    width: 100%;
    height: 100%;
  }
  [type="file"] {
    opacity: 0;
    position: absolute;
  }
}

/deep/ {
  .el-input.is-disabled .el-input__inner {
    background: transparent;
    border-color: transparent;
    color: #000;
    cursor: auto;
  }
}
</style>